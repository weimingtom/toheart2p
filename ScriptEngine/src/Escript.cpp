...
static int GetScriptParamControl(int opr)
{
	...
	if (opr < 64)
	{
		DebugPrintf("イベントスクリプトの命令コードとして不適です[%d]", opr);
	}
	opr -= 64;
	...
	for (i = 0; i < 15; i++)
	{
		...
		{
			default:
				DebugPrintf("引数タイプが不適です[%d]", EScroptOpr[opr].type[i]);
			case ESC_NOT:
				return cnt;
			case ESC_REG:
			case ESC_ADD:
	...
	for (i = 0; i < 15; i++)
	{
		...
		switch (EScroptOpr[opr].type[i])
		{
			...
			case ESC_REG:
				...
				wsprintf(buf, "REG%d, ", EscParam[i].num);
				break;
			case ESC_ADD:
				EscParam[i].num = GetScriptParamByte(pc, &cnt);
				wsprintf(buf, "ADD(%d), ", EscParam[i].num);
				break;

			case ESC_ASC:
				EscParam[i].num = GetScriptParamByte(pc, &cnt);
				wsprintf(buf, "%c, ", EscParam[i].num);
				break;
			case ESC_CNT:
				EscParam[i].num = GetScriptParamWord(pc, &cnt);
				wsprintf(buf, "CNT(%d), ", EscParam[i].num);
				break;

			case ESC_VCNT:
				EscParam[i].num = GetScriptParamWord(pc, &cnt);
				wsprintf(buf, "VCNT(%d), ", EscParam[i].num);
				break;

			case ESC_NUM:
				EscParam[i].num = GetScriptParam(pc, &cnt, 4);
				wsprintf(buf, "%d, ", EscParam[i].num);
				break;
			case ESC_STR:
			...
			case ESC_CMP:
				{
					...
					case CALL_SMALL:
						EscParam[i].num = (param1 < param3) ? 1 : 0;
						wsprintf(buf, "REG%d < %d, ", param1, param3);
						break;
					case CALL_ESMALL:
						EscParam[i].num = (param1 <= param3) ? 1 : 0;
						wsprintf(buf, "REG%d <= %d, ", param1, param3);
...
static void ESC_EOprSetChipParts(void)
{
	...
	AVG_SetChipBackParts(EscParam[0].num, EscParam[1].num, EscParam[2].num, EscParam[3].num, EscParam[4].num, EscParam[5].num, EscParam[6].num, EscParam[7].num,
	EscParam[8].num, EscParam[9].num, EscParam[10].num, DRW_PRM(EscParam[11].num, EscParam[12].num));
...
static void ESC_EOprSetSoundEvent(void)
{
	...
	AVG_SetSoundEvent(EscParam[0].num, EscParam[1].num, EscParam[2].num, EscParam[3].num, EscParam[4].num, EscParam[5].num, EscParam[6].num);
...
static void ESC_EOprSetTextFormat(void)
{
	...
	AVG_SetTextFormat(EscParam[0].num, EscParam[1].num, EscParam[2].num, EscParam[3].num, EscParam[4].num, EscParam[5].num, EscParam[6].num, EscParam[7].num,
	EscParam[8].num, EscParam[9].num, EscParam[10].num);
	EXEC_AddPC(EscCnt);
...
static void ESC_EOprSetTextEx(void)
{
	if (!EOprFlag[ESC_SETTEXTEX])
	{
		...
		if (TextExCounter > 0)
		{
			AVG_SetTextSync(TextExCounter, TextExCounter - 1);
		}
...
char NowLangFileName[64] = "";		//当前正在使用的脚本文件
EXEC_DATA *EXEC_LangInfo;
char *EXEC_LangBuf;
char LangOprList[64][32] = 
{
	"Start", "End", "MovR", "MovV", "Swap", "Rand", "IfR", "IfV", "IfElseR", "IfElseV", "Loop", "Goto", "Inc", "Dec", "Not", "Neg", "AddR", "AddV", "SubR", "SubV",
	"MulR", "MulV", "DivR", "DivV", "ModR", "ModV", "AndR", "AndV", "OrR", "OrV", "XorR", "XorV", "Calc", "Pusha", "Popa", "Call", "Ret", "Wait", "TWait", 
	"Run", "SLoad", 
};
