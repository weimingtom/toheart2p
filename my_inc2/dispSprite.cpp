...
void SPR_DrawParts(BMP_SET *vram, PARTS_STRUCT *ps, CHIP_STRUCT *cs, ANIME_CONTROL *ac, SPRITE_PARAM *sp)
{
	...
	if (ps->flag)
	{
		...
		cy = sp->dy + cy;
		{
			...
			DRAW_OBJECT dobj = DRW_SetDrawObject(vram->bmp_bit, DSP_GetBmp2(vram), cx, cy, dsx, dsy, bs->bmp_bit, DSP_GetBmp2(bs),
	ps->px, ps->py, ps->sx, ps->sy, 0, NULL, 0, 0, 0, 0, r, g, b, nuki, sp->clip, param, 0, 0, rparam);
			...
			if (dsx == ps->sx && dsy == ps->sy)
			{
				if (ac->cfade && ss->code[ac->code_no].code == SPCODE_DRAW && dobj.dparam == DRW_NML && r == 128)
				{
					...
					if (bs->bmp_bit == BMP_256P)
					{
						...
						gg = g - g * rate / 256;
						bb = b - b * rate / 256;
						dobj = DRW_SetDrawObject(vram->bmp_bit, DSP_GetBmp2(vram), cx, cy, dsx, dsy, bs->bmp_bit, DSP_GetBmp2(bs), ps->px,
	ps->py, ps->sx, ps->sy, 0, NULL, 0, 0, 0, 0, rr, gg, bb, nuki, sp->clip, param, 0, 0, rparam);
						switch (vram->bmp_bit)
						{
							case BMP_256P:
								break;
						...
						gg = g *(rate) / 256;
						bb = b *(rate) / 256;
						dobj = DRW_SetDrawObject(vram->bmp_bit, DSP_GetBmp2(vram), cx, cy, dsx, dsy, bs2->bmp_bit, DSP_GetBmp2(bs2), ps2
	->px, ps2->py, ps2->sx, ps2->sy, 0, NULL, 0, 0, 0, 0, rr, gg, bb, nuki, sp->clip, DRW_ADD, 0, 0, rparam);
						switch (vram->bmp_bit)
						{
							case BMP_256P:
								break;
					...
					{
						dobj = DRW_SetDrawObject(vram->bmp_bit, DSP_GetBmp2(vram), cx, cy, dsx, dsy, bs->bmp_bit, DSP_GetBmp2(bs), ps->px,
	ps->py, ps->sx, ps->sy, 0, NULL, 0, 0, 0, 0, r, g, b, nuki, sp->clip, DRW_NML, 0, 0, rparam);
						switch (vram->bmp_bit)
						{
							case BMP_256P:
								break;
						...
						dobj = DRW_SetDrawObject(vram->bmp_bit, DSP_GetBmp2(vram), cx, cy, dsx, dsy, bs2->bmp_bit, DSP_GetBmp2(bs2), ps2
	->px, ps2->py, ps2->sx, ps2->sy, 0, NULL, 0, 0, 0, 0, r, g, b, nuki, sp->clip, DRW_BLD(rate), 0, 0, rparam);
						switch (vram->bmp_bit)
						{
							case BMP_256P:
								break;
...
void SPR_DrawSprite(void *dest, int draw_mode, int ani_no, int dx, int dy, int r, int g, int b, RECT *clip, int param,
	int rparam, int zoom)
{
	BMP_SET vram;
	SPRITE_STRUCT *ss;
	...
	SPRITE_PARAM sp = 
	{
		dx, dy, r, g, b, clip, param, rparam, zoom
	};

	if (!ac->flag)
	{
		DebugPrintf("AnimeControlに登録されていません[%d]\n", ani_no);
		return ;
	}
	if (!AnimeStruct[ac->ano].flag)
	{
		DebugPrintf("AnimeStructに登録されていません[%d]\n", ac->ano);
		return ;
	}
...
static void RenewSprite(SPRITE_OPR code[], long *count, long *code_no, long *draw_count, char *loop_no, char lcount[],
	ANIME_CONTROL *ac)
{
	int i;
	while (*draw_count > 0)
			...
			case SPCODE_REPEAT:
				...
				else
				{
					...
					case 0:
						...
						i =  *code_no;
						while (i >= 0 && code[i].code != SPCODE_LOOP)
						{
							i--;
						}
						*code_no = i + 1;
...
int SPR_CopyBmp(int src_bno, char *tfname, int viv, int r, int g, int b)
{
	...
	{
		DebugPrintf("SPBMP領域エラー[SRC=%d]", src_bno);
		return  - 1;
	}
	if (dst_bno < 0 || dst_bno > SPBMP_MAX)
	{
		DebugPrintf("SPBMP領域エラー[DST=%d]", dst_bno);
		return  - 1;
	}
	...
	if (!src_bmp->flag)
	{
		DebugPrintf("ソースの殪い画像コピーを実行しました");
		return  - 1;
	}
	...
	if (dst_bno != src_bno)
	{
		...
		switch (src_bmp->bmp_bit)
		{
			...
			case 1:
				...
				dobj = DRW_SetDrawObject(BMP_HIGH, &dst_bmp->bmp_h, 0, 0, src_bmp->bmp_h.sx, src_bmp->bmp_h.sy, BMP_HIGH, &src_bmp
	->bmp_h, 0, 0, src_bmp->bmp_h.sx, src_bmp->bmp_h.sy, 0, NULL, 0, 0, 0, 0, r, g, b, CHK_NO, (RECT*) - 1, DRW_NML, 0,
	0, 0);
				DRW_DrawBMP_HH(dobj);
			...
			case 2:
				...
				dobj = DRW_SetDrawObject(BMP_FULL, &dst_bmp->bmp_f, 0, 0, dst_bmp->bmp_f.sx, dst_bmp->bmp_f.sy, BMP_FULL, &src_bmp
	->bmp_f, 0, 0, src_bmp->bmp_f.sx, src_bmp->bmp_f.sy, 0, (RECT*)NULL, 0, 0, 0, 0, r, g, b, CHK_NO, (RECT*) - 1,
	DRW_NML, 0, 0, 0);
				DRW_DrawBMP_FF(dobj);
			...
			case 3:
				...
				dobj = DRW_SetDrawObject(BMP_TRUE, &dst_bmp->bmp_t, 0, 0, src_bmp->bmp_t.sx, src_bmp->bmp_t.sy, BMP_TRUE, &src_bmp
	->bmp_t, 0, 0, src_bmp->bmp_t.sx, src_bmp->bmp_t.sy, 0, (RECT*)NULL, 0, 0, 0, 0, r, g, b, CHK_NO, (RECT*) - 1,
	DRW_NML, 0, 0, 0);
				DRW_DrawBMP_TT(dobj);
...
BOOL SPR_SetSprite(int ani_no, int sp_no, int mode, int end, int lnum, int cfade)
{
	...
	{
		DebugPrintf("スプライトの表示数が飽和しました[max=%d]", ANIME_CONTROL_MAX);
		return  - 1;
	}
	if (ani_no < 0 || ANIME_MAX <= ani_no)
	{
		DebugPrintf("存在しないスプライト領域が指定されました[%d]\n", ani_no);
		return  - 1;
	}
...
BOOL SPR_SetSpritePlay(int ac_no, int ani_no, int sp_no, int mode = SP_PLAY, int end = ON, int lnum = 0, int cfade = OFF)
{
	...
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります[%d]\n", ac_no);
		return FALSE;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました[%d]\n", ac_no);
		return FALSE;
	}
	if (ani_no < 0 || ANIME_MAX <= ani_no)
	{
...
BOOL SPR_SetSpriteNo(int ac_no, int sp_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります1[%d]\n", ac_no);
		return FALSE;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました1[%d]\n", ac_no);
		return FALSE;
	}
	ANIME_CONTROL *ac = &AnimeControl[ac_no];
...
BOOL SPR_SetSpriteCfade(int ac_no, int flag)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります2[%d]\n", ac_no);
		return FALSE;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました2[%d]\n", ac_no);
		return FALSE;
	}
...
BOOL SPR_SetSpriteLoop(int ac_no, int end, int lnum)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります3[%d]\n", ac_no);
		return FALSE;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました3[%d]\n", ac_no);
		return FALSE;
	}
...
BOOL SPR_SetSpriteLoopThrough(int ac_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります4[%d]\n", ac_no);
		return FALSE;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました4[%d]\n", ac_no);
		return FALSE;
	}
...
BOOL SPR_SetSpriteRepeatFind(int ac_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります5[%d]\n", ac_no);
		return FALSE;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました5[%d]\n", ac_no);
		return FALSE;
	}
...
int SPR_GetSpriteRepeatFind(int ac_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります6[%d]\n", ac_no);
		return REPEAT_NULL;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました6[%d]\n", ac_no);
		return REPEAT_NULL;
	}
...
RECT *SPR_GetSpriteRect(int ac_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります7[%d]\n", ac_no);
		return NULL;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました7[%d]\n", ac_no);
		return NULL;
	}
...
BOOL SPR_WaitSpriteCondition(int ac_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります8[%d]\n", ac_no);
		return FALSE;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました8[%d]\n", ac_no);
		return FALSE;
	}
...
int SPR_GetSpriteMode(int ac_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります9[%d]\n", ac_no);
		return SP_STOP;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました9[%d]\n", ac_no);
		return SP_STOP;
	}
...
BOOL SPR_SetSpriteMode(int ac_no, int mode)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります10[%d]\n", ac_no);
		return SP_STOP;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました10[%d]\n", ac_no);
		return SP_STOP;
	}
...
int SPR_GetSpriteEvent(int ac_no)
{
	if (ac_no < 0 || ANIME_CONTROL_MAX <= ac_no)
	{
		DebugPrintf("スプライト制御バッファの指定に誤りがあります11[%d]\n", ac_no);
		return 0;
	}
	if (AnimeControl[ac_no].flag == 0)
	{
		DebugPrintf("存在しないスプライト制御バッファが指定されました11[%d]\n", ac_no);
		return 0;
	}
	int event = 0;
