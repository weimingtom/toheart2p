...
enum SoundBufType
{
	memory_sound = 0, stream_sound, 
};


enum CL_PCMSTATUS
{
	PCM_STOP = 0, PCM_PLAY, PCM_FADEOUT, PCM_FADEOUTBREAK, 
};
...
struct ClSoundBuffer
{
	...
	int nextStream;
	int orgMusicNum;
	HANDLE hLoopThread;				//循环播放线程
	HANDLE hFadeThread;				//淡入淡出线程
	HANDLE hEvent[3];				//多线同步Event数组
	...
	int nextArcFileNum;				//下一个arc文件的索引号
	int nextMusicNum;				//下一个音乐索引号
	int nextRepeat;
	int endPt;
...
class ClSoundDS
{
	...
	public:
		...
		inline LPDIRECTSOUNDBUFFER GetPrimarySoundBuffer(void)
		{
