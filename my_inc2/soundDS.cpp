...
ClResult ClSoundDS::GetPlayStreamVolume(int &handle, DWORD volme[2])
{
	...
	if (lpSoundBuffer)
	{
		...
		if (status == PCM_PLAY)
		{
			...
			{
				if (lpSoundBuffer->lpDSBuffer->Lock(CurrentWriteCursor, BAR_GET_SIZE, (void **) &lpPtr1, &dwLen1, (void **) &lpPtr2,
	&dwLen2, 0) == DS_OK)
				{
...
ClResult ClSoundDS::play(int handle, int repeat)
{
	...
	{
		lpReadFile->StreamSeekFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpSoundBuffer->dataTopOffset,
	FILE_BEGIN);

		char *lpBlockAdd1,  *lpBlockAdd2;
		DWORD blockSize1, blockSize2;

		hr = lpSoundBuffer->lpDSBuffer->Lock(0, lpSoundBuffer->streamBlockSize, (LPVOID*) &lpBlockAdd1, &blockSize1, (LPVOID*)
	&lpBlockAdd2, &blockSize2, 0);
		if (DS_OK == hr)
		{
			if (WAVE_FORMAT_OGG == lpSoundBuffer->wFormatTag)
			{
				...
				while (remainSize >= outsize && remainSize > 0)
				{
					...
					if (ret == OGG_DEC_END && lpSoundBuffer->nextStream >= 0)
					{
						...
						lpSoundBuffer->streamNum = lpSoundBuffer->readFile->StreamOpenFileNum(lpSoundBuffer->arcFileNum, lpSoundBuffer
	->nextStream, size);
			...
			else if (NULL == lpSoundBuffer->hAcm)
			{
				...
				if (lpSoundBuffer->streamBlockSize > (int)blockSize1)
				{
			...
			{
				lpReadFile->StreamReadFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, (char*)lpSoundBuffer
	->acmSrc.lpStream, lpSoundBuffer->acmSrc.dwStreamSize);
				...
				if (0 == nError)
				{
					...
					while (remainSize >= (int)lpSoundBuffer->ash.cbDstLengthUsed && remainSize > 0)
					{
...
void ClSoundDS::NextMusicStart()
{
	...
	while (lpNext)
	{
		...
		if (lpSoundBuffer && lpSoundBuffer->nextMusicNum >= 0)
		{
			...
			BOOL ret = lpSoundDS->CreateStreamBuffer(lpSoundBuffer->handle, lpSoundBuffer->nextArcFileNum, lpSoundBuffer
	->nextMusicNum);
...
BOOL ClSoundDS::CreateStreamBuffer(int handle, int arcFileNum, int playNum)
{
	...
	else if (pcmFormat.wf.wFormatTag != WAVE_FORMAT_PCM)
	{
		...
		mmResult = acmFormatSuggest(NULL, pwfxInfo,  &dstWF, sizeof(dstWF), ACM_FORMATSUGGESTF_WFORMATTAG);
		if (mmResult != 0)
		{
			return FALSE;
		}
		...
		acmStreamSize(lpDSBufferTail->hAcm, lpDSBufferTail->acmDst.dwStreamSize, &lpDSBufferTail->acmSrc.dwStreamSize,
	ACM_STREAMSIZEF_DESTINATION);
		acmStreamSize(lpDSBufferTail->hAcm, lpDSBufferTail->acmSrc.dwStreamSize, &lpDSBufferTail->acmDst.dwStreamSize,
	ACM_STREAMSIZEF_SOURCE);
		cl_free(pwfxInfo);
		if (fmtHead.size < (int)lpDSBufferTail->acmSrc.dwStreamSize *2)
		{
	...
	else
	{
		...
		if (fmtHead.size < (int)pcmFormat.wf.nAvgBytesPerSec *2)
		{
			lpReadFile->StreamCloseFile(arcFileNum, stream);
			return  - 1;
		}
...
BOOL ClSoundDS::CreateDummyBuffer()
{
	...
	if (hr == DS_OK)
	{
		...
		lpDummyBuffer->Lock(0, dsbdesc.dwBufferBytes, (LPVOID*) &lpBlockAdd1, &blockSize1, (LPVOID*) &lpBlockAdd2, &blockSize2,
	0);
...
BOOL ClSoundDS::AddSoundBuffer(int handle, char *fileBuf, BOOL b3DSound)
{
	...
	if (pcmFormat.wf.wFormatTag == WAVE_FORMAT_PCM)
	{
		...
		lpTmpBuffer->QueryInterface(IID_IDirectSoundBuffer8, (LPVOID*) &lpDSBufferTail->lpDSBuffer);
		RELEASE(lpTmpBuffer);
		hr = lpDSBufferTail->lpDSBuffer->Lock(0, fmtHead.size, (LPVOID*) &lpBlockAdd1, &blockSize1, (LPVOID*) &lpBlockAdd2,
	&blockSize2, 0);
		if (DS_OK == hr)
		{
			CopyMemory(lpBlockAdd1, fileBuf, blockSize1);
			if (fmtHead.size > (int)blockSize1)
			{
	...
	else
	{
		...
		lpTmpBuffer->QueryInterface(IID_IDirectSoundBuffer8, (LPVOID*) &lpDSBufferTail->lpDSBuffer);
		RELEASE(lpTmpBuffer);
		hr = lpDSBufferTail->lpDSBuffer->Lock(0, ash.cbDstLengthUsed, (LPVOID*) &lpBlockAdd1, &blockSize1, (LPVOID*)
	&lpBlockAdd2, &blockSize2, 0);
		if (DS_OK == hr)
		{
...
//			int				handle			声音的通道号
//返回值：	
//			ClSoundBuffer *	lpSoundBuffer	返回指定通道的声音buffer
...
void ClSoundDS::Set3DListener(float x, float y, float z, float deg)
{
	...
	if (lpSoundDS->lpListener)
	{
		...
		lpSoundDS->lpListener->SetOrientation(sinf(DEGtoRAD(deg)), 0, cosf(DEGtoRAD(deg)), 0, 1.0f, 0, DS3D_IMMEDIATE);
	}
}
...
static DWORD WINAPI StreamLoop(LPVOID param)
{
	...
	while (TRUE)
	{
		...
		switch (pt)
		{
			case (WAIT_OBJECT_0): case (WAIT_OBJECT_0 + 1): size = pt - WAIT_OBJECT_0 + 1;
			if (size > 1)
			{
				size = 0;
			}
			...
			hr = lpSoundBuffer->lpDSBuffer->Lock(lpSoundBuffer->streamBlockSize *size, lpSoundBuffer->streamBlockSize, (LPVOID*)
	&lpBlockAdd1, &blockSize1, (LPVOID*) &lpBlockAdd2, &blockSize2, 0);
			if (DS_OK == hr)
			{
				if (WAVE_FORMAT_OGG == lpSoundBuffer->wFormatTag)
				{
					...
					{
						if (lpSoundBuffer->leakSize > (int)blockSize1)
						{
					...
					if (ret == OGG_DEC_END)
					{
						oggFilEnd: if (1 == lpSoundBuffer->repeat)
						{
	...
	->nextStream, size__);
							...
							lpSoundBuffer->readFile->StreamSeekFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpSoundBuffer
	->dataTopOffset, FILE_BEGIN);
							while (remainSize >= outsize && remainSize > 0)
							{
								CopyMemory(lpBlockAdd1 + writeSize, outBuf, outsize);
								remainSize -= outsize;
								...
								if (remainSize > 0)
								{
									rsize = lpSoundBuffer->readFile->StreamReadFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, workBuf,
	4096);
									ret = lpSoundBuffer->o_dat->DecodeOGG(workBuf, rsize, outBuf, 0x20000, &outsize);
									if (OGG_DEC_END == ret)
									{
	...
	4096);
				...
				{
					size = lpSoundBuffer->readFile->StreamReadFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpBlockAdd1,
	blockSize1);
					if (size < blockSize1)
					{
						...
						else
						{
							...
							lpSoundBuffer->readFile->StreamSeekFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpSoundBuffer
	->dataTopOffset, FILE_BEGIN);
							lpSoundBuffer->readFile->StreamReadFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpBlockAdd1 + size,
	blockSize1 - size);
						}
					}
					else
					{
						if (lpSoundBuffer->streamBlockSize > (int)blockSize1)
						{
							size = lpSoundBuffer->readFile->StreamReadFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpBlockAdd2,
	blockSize2);
							if (size < blockSize2)
							{
								if (1 == lpSoundBuffer->repeat)
								...
								else
								{
									...
									lpSoundBuffer->readFile->StreamSeekFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpSoundBuffer
	->dataTopOffset, FILE_BEGIN);
									lpSoundBuffer->readFile->StreamReadFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, lpBlockAdd2 + size,
	blockSize2 - size);
								}
							}
						}
				...
				else
				{
					...
					size = lpSoundBuffer->readFile->StreamReadFile(lpSoundBuffer->arcFileNum, lpSoundBuffer->streamNum, (char*)
	lpSoundBuffer->acmSrc.lpStream, lpSoundBuffer->acmSrc.dwStreamSize);
					if (size < lpSoundBuffer->acmSrc.dwStreamSize)
					{
						...
						if (1 == lpSoundBuffer->repeat)
						{
							if (0 == nError)
							{
								if (remainSize >= (int)lpSoundBuffer->ash.cbDstLengthUsed)
								{
									CopyMemory(lpBlockAdd1 + writeSize, lpSoundBuffer->acmDst.lpStream, lpSoundBuffer->ash.cbDstLengthUsed);
									writeSize += lpSoundBuffer->ash.cbDstLengthUsed;
									remainSize -= lpSoundBuffer->ash.cbDstLengthUsed;
								...
								{
									CopyMemory(lpBlockAdd1 + writeSize, lpSoundBuffer->acmDst.lpStream, remainSize);
									CopyMemory(lpBlockAdd2, lpSoundBuffer->acmDst.lpStream + remainSize, lpSoundBuffer->ash.cbDstLengthUsed -
	remainSize);
									if (blockSize2 > lpSoundBuffer->ash.cbDstLengthUsed - remainSize)
									{
										ZeroMemory(lpBlockAdd2 + (lpSoundBuffer->ash.cbDstLengthUsed - remainSize), blockSize2 - (lpSoundBuffer
	->ash.cbDstLengthUsed - remainSize));
									}
	...
	lpSoundBuffer->acmSrc.lpStream, lpSoundBuffer->acmSrc.dwStreamSize);
			...
			break;
			default:
				SoundStop: lpSoundBuffer->repeat = 0;
