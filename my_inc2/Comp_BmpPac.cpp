	...
	else
	{
		switch (draw_mode)
		{
			...
			case 16:
				...
				dsize = LZS_DecodeMemoryBmpTbl_WORD(bmp_h->buf + buf_point, mi->buf, now_esize, mi->bmi.sabun, bai, mi->r, mi->g, mi
	->b);
				break;
			case 24:
				buf_point = (bmp_f->sy - mi->bmi.sy *((bai) ? 2 : 1)) / 2 * bmp_f->sx;
				dsize = LZS_DecodeMemoryBmpTbl_3BYTE((char*)bmp_f->buf + buf_point, mi->buf, now_esize, mi->bmi.sabun, bai, mi->r,
	mi->g, mi->b);

				break;
			case 32:
				buf_point = (bmp_t->sy - mi->bmi.sy *((bai) ? 2 : 1)) / 2 * bmp_t->sx;
				dsize = LZS_DecodeMemoryBmpTbl_DWORD(bmp_t->buf + buf_point, mi->buf, now_esize, mi->bmi.sabun, bai, mi->r, mi->g,
	mi->b);
				break;
		}
	}
		...
		if (!sabun)
		{
			...
			for (;; flags >>= 1)
			{
				...
				else
				{
					...
					{
						text_buf[r &(RBUF_SIZE - 1)] = text_buf[c &(RBUF_SIZE - 1)];
						*(RGB32*)(dst2 + 3) = *(RGB32*)dst2 = *(RGB32*)(dst2 + 1923) = *(RGB32*)(dst2 + 1920) = tbl[text_buf[c &(RBUF_SIZE
	- 1)]];
			...
			else
			{
				...
				{
					text_buf[r &(RBUF_SIZE - 1)] = text_buf[c &(RBUF_SIZE - 1)];
					*(WORD*)(dst2 + 3) = *(WORD*)dst2 = *(WORD*)(dst2 + 641 * 3) = *(WORD*)(dst2 + 640 * 3) = *(WORD*) &tbl[text_buf[c
	&(RBUF_SIZE - 1)]];
			...
			for (;; flags >>= 1)
			{
				...
				else
				{
					...
					for (; j >= 0; j--, c++)
					{
						...
						if (text_buf[c &(RBUF_SIZE - 1)] != 0xff)
						{
							*(WORD*)(dst2 + 3) = *(WORD*)dst2 = *(WORD*)(dst2 + 641 * 3) = *(WORD*)(dst2 + 640 * 3) = *(WORD*)
	&tbl[text_buf[c &(RBUF_SIZE - 1)]];
							*(dst2 + 3+2) = *(dst2 + 2) = *(dst2 + 641 * 3+2) = *(dst2 + 640 * 3+2) = tbl[text_buf[c &(RBUF_SIZE - 1)]].r;
						}
