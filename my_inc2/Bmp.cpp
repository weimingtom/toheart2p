...
{
	{
		sizeof(BITMAPINFOHEADER), 640, 480, 1, 8, BI_RGB, 640 *480, 0, 0, 256, 0, 
	}
	, 
...
{
	{
		sizeof(BITMAPINFOHEADER), 640, 480, 1, 16, BI_BITFIELDS, 640 *480 * 2, 0, 0, 0, 0, 
	}
...
{
	{
		sizeof(BITMAPINFOHEADER), 640, 480, 1, 24, BI_RGB, 0, 0, 0, 0, 0, 
	}
	, 
	{
		0xff, 0xff, 0xff
...
{
	{
		sizeof(BITMAPINFOHEADER), 640, 480, 1, 32, BI_RGB, 0, 0, 0, 0, 0, 
	}
...
BOOL BMP_SetTonecurve_T(BMP_T *bmp_t, char *fname, int viv)
{
	...
	if (viv != 256)
	{
		...
		if (viv == 0)
		{
			for (i = 0; i < size2; i++)
			{
				bmp_t->buf[i].r = bmp_t->buf[i].g = bmp_t->buf[i].b = (bmp_t->buf[i].r *77+bmp_t->buf[i].g *28+bmp_t->buf[i].b *151)
	>> 8;
			}
		}
		else
...
BOOL BMP_SetTonecurve_F(BMP_F *bmp_f, char *fname, int viv)
{
	...
	if (viv != 256)
	{
		...
		if (viv == 0)
		{
			for (i = 0; i < size; i++)
			{
				bmp_f->buf[i].r = bmp_f->buf[i].g = bmp_f->buf[i].b = (bmp_f->buf[i].r *77+bmp_f->buf[i].g *28+bmp_f->buf[i].b *151)
	>> 8;
			}
		}
		else
...
BOOL BMP_SetTonecurve_H(BMP_H *bmp_h, char *fname, int viv)
{
	...
	if (size == 1280)
	{
		...
		if (viv == 256)
		{
			...
			{
				bmp_h->buf[i] = (rgb[r[((bmp_h->buf[i] &rgb16.mask_r) >> rgb16.pos_r << rgb16.num_r)]] >> rgb16.num_r << rgb16.pos_r)
	| (rgb[g[((bmp_h->buf[i] &rgb16.mask_g) >> rgb16.pos_g << rgb16.num_g)]] >> rgb16.num_g << rgb16.pos_g) | (rgb[b[(
	(bmp_h->buf[i] &rgb16.mask_b) >> rgb16.pos_b << rgb16.num_b)]] >> rgb16.num_b << rgb16.pos_b);
			}
		}
		else
		{
			...
			for (i = 0; i < bmp_h->sy *bmp_h->sx; i++)
			{
				...
				bmp_h->buf[i] = (rgb[r[(glay + rr * viv) >> 8]] >> rgb16.num_r << rgb16.pos_r) | (rgb[g[(glay + gg * viv) >> 8]] >>
	rgb16.num_g << rgb16.pos_g) | (rgb[b[(glay + bb * viv) >> 8]] >> rgb16.num_b << rgb16.pos_b);
			}
		}
	}
	else if (size == 768)
	{
		...
		if (viv == 256)
		{
			for (i = 0; i < bmp_h->sy *bmp_h->sx; i++)
			{
				bmp_h->buf[i] = (r[((bmp_h->buf[i] &rgb16.mask_r) >> rgb16.pos_r << rgb16.num_r)] >> rgb16.num_r << rgb16.pos_r) | 
	(g[((bmp_h->buf[i] &rgb16.mask_g) >> rgb16.pos_g << rgb16.num_g)] >> rgb16.num_g << rgb16.pos_g) | (b[((bmp_h
	->buf[i] &rgb16.mask_b) >> rgb16.pos_b << rgb16.num_b)] >> rgb16.num_b << rgb16.pos_b);
			}
		}
		else
		{
			...
			for (i = 0; i < bmp_h->sy *bmp_h->sx; i++)
			{
				...
				bmp_h->buf[i] = (r[(glay + rr * viv) >> 8] >> rgb16.num_r << rgb16.pos_r) | (g[(glay + gg * viv) >> 8] >>
	rgb16.num_g << rgb16.pos_g) | (b[(glay + bb * viv) >> 8] >> rgb16.num_b << rgb16.pos_b);
			}
		}
	}
	else if (size == 256)
	{
		...
		if (viv == 256)
		{
			for (i = 0; i < bmp_h->sy *bmp_h->sx; i++)
			{
				bmp_h->buf[i] = (rgb[((bmp_h->buf[i] &rgb16.mask_r) >> rgb16.pos_r << rgb16.num_r)] >> rgb16.num_r << rgb16.pos_r) |
	(rgb[((bmp_h->buf[i] &rgb16.mask_g) >> rgb16.pos_g << rgb16.num_g)] >> rgb16.num_g << rgb16.pos_g) | (rgb[((bmp_h
	->buf[i] &rgb16.mask_b) >> rgb16.pos_b << rgb16.num_b)] >> rgb16.num_b << rgb16.pos_b);
			}
		}
		else
		{
			...
			for (i = 0; i < bmp_h->sy *bmp_h->sx; i++)
			{
				...
				bmp_h->buf[i] = (rgb[(glay + rr * viv) >> 8] >> rgb16.num_r << rgb16.pos_r) | (rgb[(glay + gg * viv) >> 8] >>
	rgb16.num_g << rgb16.pos_g) | (rgb[(glay + bb * viv) >> 8] >> rgb16.num_b << rgb16.pos_b);
			}
		}
	}
	BMP_ReleaseTonecurve();
...
//输入参数：			
//			类型		参数名				意义
//			char *		fname				要加载的BMP文件名
//			BMP_SET *	bmp_set（输入输出）	BMP集合结构体地址
//			char *		pack   （输出）		取得的保存BMP文件的文件名
//返回值：
//			BOOL		ret					成功TRUE；失败FALSE
BOOL BMP_LoadBMP_X2X(char *fname, BMP_SET *bmp_set, char *pack)
{
	BITMAPFILEHEADER *BMPFileHead;
	BITMAPINFOHEADER *BMPFileInfo;
	char *buf,  *ptr;				//读出文件的地址
	int rev_bmp = 1, y, i;
	int palnum;
	BOOL ret = FALSE;
		...
		switch (bpp)
		{
			case 8:
				...
				else
				{
					...
					{
						CopyMemory(bmp_set->bmp_b.buf + bmp_set->bmp_b.sx *(bmp_set->bmp_b.sy - y - 1), bmp + bmp_set->bmp_b.sx *y *
	sizeof(BYTE), bmp_set->bmp_b.sx *sizeof(BYTE));
					}
				}
				for (i = 0; i < palnum; i++)
				{
			...
			case 24:
				...
				else
				{
					...
					{
						CopyMemory(bmp_set->bmp_f.buf + bmp_set->bmp_f.sx *y, bmp + bmp_set->bmp_f.sx *sizeof(RGB24)*(bmp_set->bmp_f.sy -
	y - 1), bmp_set->bmp_f.sx *sizeof(RGB24));
					}
				}
				break;
			case 32:
				...
				else
				{
					...
					{
						CopyMemory(bmp_set->bmp_t.buf + bmp_set->bmp_t.sx *y, bmp + bmp_set->bmp_t.sx *sizeof(RGB32)*(bmp_set->bmp_t.sy -
	y - 1), bmp_set->bmp_t.sx *sizeof(RGB32));
					}
				}
				break;
		}
	...
	else
	{
		...
		switch (BMPFileInfo->biBitCount)//当前BMP图片颜色的位率
		{
			default:
				DebugBox(NULL, "BMPデータのBIT数が異常です[%s](%dBIT?)", fname, BMPFileInfo->biBitCount);
				break;
			case 8:
			...
			case 24:
				...
				bmp_set->bmp_f.buf = (RGB24*)GlobalAlloc(GPTR, bmp_set->bmp_f.sx *bmp_set->bmp_f.sy *sizeof(RGB24));
				ptr += palnum;
				//读入BMP文件到BMP集合体bmp_set中，ptr此时为BMP数据
...
//-------------------------------------------------------------------------
//函数名：	BMP_CreateBMP_F
//功能：	分配新的内存空间给24位的BMP位图　
//
//输入参数：			
//			类型		参数名			意义
//			BMP_F *		bmp_f			要分配位图数据的内存地址
//			int			sx				位置x
//			int			sy				位置y
//返回值：
//			无
...
//-------------------------------------------------------------------------
//函数名：	BMP_ReleaseBMP_F
//功能：	释放24位BMP图像地址的数据
//
//输入参数：			
//			类型		参数名			意义
//			BMP_F*		bmp_f			要释放的BMP集合结构体地址
//返回值：
//		无
...
int BMP_EncodeVisual(char *dst_fname, char *src_fname, char *dir)
{
	...
	BMPFileHead.bfSize = BMPFileInfo.biWidth * abs(BMPFileInfo.biHeight) *3+sizeof(BITMAPFILEHEADER) + sizeof
	(BITMAPINFOHEADER);
	BMPFileHead.bfOffBits = (sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER));
