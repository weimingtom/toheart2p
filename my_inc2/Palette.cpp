	...
	for (src = 0; src < num; src++)
	{
		if (lpChgTbl[src])
		{
			for (dst = 1; dst < 256; dst++)
			{
				if (lpUseTbl[dst])
				{
					if ((rgbPal[src].r == lpPalData[dst].peRed) && (rgbPal[src].g == lpPalData[dst].peGreen) && (rgbPal[src].b ==
	lpPalData[dst].peBlue))
					{
						tbl[src] = dst;
						lpChgTbl[src] = 0;
						break;
	...
	if ((src < num) && (dst >= 256))
	{
		...
		for (; src < num; src++)
		{
			if (lpChgTbl[src])
			{
				...
				for (dst = 1; dst < 256; dst++)
				{
					val = abs(rgbPal[src].r - lpPalData[dst].peRed) + abs(rgbPal[src].g - lpPalData[dst].peGreen) + abs(rgbPal[src].b -
	lpPalData[dst].peBlue);
					if (val < min)
					{
						min = val;
