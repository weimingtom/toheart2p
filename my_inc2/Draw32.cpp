...
BOOL DRW_DrawBMP_TTT(DRAW_OBJECT dobj)
{
	...
	if (dest->buf == NULL)
	{
		DebugPrintf("Ãè»­îIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawBMP_TTT]\n");
		return FALSE;
	}
	if (src1->buf == NULL)
	{
		DebugPrintf("»­ÏñîIÓò1¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawBMP_TTT]\n");
		return FALSE;
	}
	if (src2->buf == NULL)
	{
		DebugPrintf("»­ÏñîIÓò2¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawBMP_TTT]\n");
		return FALSE;
	}
	...
	if (dobj.clip != (void*) - 1)
	{
		if (!ClipRectS2(&dobj.dx, &dobj.dy, dest->sx, dest->sy,  &dobj.s1x, &dobj.s1y, src1->sx, src1->sy, &dobj.s1w, &dobj.s1h,  &dobj.s2x, &dobj.s2y, src2->sx,
			src2->sy, &dobj.s2w, &dobj.s2h, dobj.clip))
		{
			return FALSE;
		}
	}
	...
	case DRW_AMI2:
		...
		break;
	case DRW_LPP2:
	case DRW_LPP2 + 1: case DRW_LPP2 + 2: case DRW_LPP2 + 3: case DRW_LPP2 + 4: case DRW_LPP2 + 5: if (dobj2.dest)
					   {
...
BOOL DRW_DrawBMP_TT_Sui(DRAW_OBJECT *dobj)
{
	...
	for (y = 0, yy = yyy; y < 8; y++, yy += yinc)
	{
		...
		for (x = 0, xx = xxx; x < 8; x++, xx += xinc)
		{
			...
			dobj2 = DRW_SetDrawObject(3, dest, xx *w2, yy *h2, w2, h2, 3, src, xx *w2 + x1, yy *h2 + y1, w2 + x2 - x1, h2 + y2 - y1, 0, NULL, 0, 0, 0, 0, dobj->r, dobj
				->g, dobj->b, dobj->nuki, dobj->clip, DRW_NML, 0, 0, dobj->rparam);
...
BOOL DRW_DrawBMP_TT_Sbl(DRAW_OBJECT *dobj)
{
	...
	for (y = 0, yy = yyy; y < 8; y++, yy += yinc)
	{
		...
		for (x = 0, xx = xxx; x < 8; x++, xx += xinc)
		{
			...
			dobj2 = DRW_SetDrawObject(3, dest, xx *w2, yy *h2, w2, h2, 3, src, xx *w2 + x1, yy *h2 + y1, w2 + x2 - x1, h2 + y2 - y1, 0, NULL, 0, 0, 0, 0, dobj->r, dobj
				->g, dobj->b, dobj->nuki, dobj->clip, DRW_BLD(32), 0, 0, dobj->rparam);
...
BOOL DRW_DrawBMP_TT(DRAW_OBJECT dobj)
{
	...
	if (dest->buf == NULL)
	{
		DebugPrintf("Ãè»­îIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawBMP_TT]\n");
		return FALSE;
	}
	if (src->buf == NULL)
	{
		DebugPrintf("»­ÏñîIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawBMP_TT]\n");
		return FALSE;
	}
	...
	if (dobj.clip != (void*) - 1)
	{
		if (!ClipRect(&dobj.dx, &dobj.dy, dest->sx, dest->sy,  &dobj.s1x, &dobj.s1y, src->sx, src->sy, &dobj.dw, &dobj.dh, dobj.clip))
		{
			return FALSE;
		}
	}
	...
	case DRW_LCF2:
	case DRW_LCF2 + 1: case DRW_LCF2 + 2: case DRW_LCF2 + 3: case DRW_LCF2 + 4: case DRW_LCF2 + 5: DRW_DrawBMP_TT_Lcf(&dobj);
		break;

	case DRW_LPP2:
	case DRW_LPP2 + 1: case DRW_LPP2 + 2: case DRW_LPP2 + 3: case DRW_LPP2 + 4: case DRW_LPP2 + 5: DRW_DrawBMP_TT_Lpp(&dobj);
		break;

	case DRW_DIA2:
	case DRW_DIA2 + 1: case DRW_DIA2 + 2: case DRW_DIO2:
	case DRW_DIO2 + 1: case DRW_DIO2 + 2: DRW_DrawBMP_TT_Dia(&dobj);
		break;
	}
	return ret;
}
...
BOOL DRW_DrawBMP_TB_Sui(DRAW_OBJECT *dobj, BMP_BT *src)
{
	...
	for (y = 0, yy = yyy; y < 8; y++, yy += yinc)
	{
		...
		for (x = 0, xx = xxx; x < 8; x++, xx += xinc)
		{
			...
			dobj2 = DRW_SetDrawObject(3, dest, xx *w2, yy *h2, w2, h2, 0, src, xx *w2 + x1, yy *h2 + y1, w2 + x2 - x1, h2 + y2 - y1, 0, NULL, 0, 0, 0, 0, dobj->r, dobj
				->g, dobj->b, dobj->nuki, dobj->clip, DRW_NML, 0, 0, dobj->rparam);
...
BOOL DRW_DrawBMP_TB_Sbl(DRAW_OBJECT *dobj, BMP_BT *src)
{
	...
	for (y = 0, yy = yyy; y < 8; y++, yy += yinc)
	{
		...
		for (x = 0, xx = xxx; x < 8; x++, xx += xinc)
		{
			...
			dobj2 = DRW_SetDrawObject(3, dest, xx *w2, yy *h2, w2, h2, 0, src, xx *w2 + x1, yy *h2 + y1, w2 + x2 - x1, h2 + y2 - y1, 0, NULL, 0, 0, 0, 0, dobj->r, dobj
				->g, dobj->b, dobj->nuki, dobj->clip, DRW_BLD(128), 0, 0, dobj->rparam);
...
BOOL DRW_DrawBMP_TB(DRAW_OBJECT dobj)
{
	...
	BMP_BT src2 = 
	{
		src->buf, src->alp, pal, src->sx, src->sy
	};


	if (dest->buf == NULL)
	{
		DebugPrintf("Ãè»­îIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawBMP_TB]\n");
		return FALSE;
	}
	if (src->buf == NULL)
	{
		DebugPrintf("»­ÏñîIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawBMP_TB]\n");
		return FALSE;
	}
	...
	if (dobj.r == 128 && dobj.g == 128 && dobj.b == 128 && !(dobj.dparam == DRW_BLD2 || dobj.dparam == DRW_NEG || dobj.dparam == DRW_SBL2 || dobj.dparam ==
		DRW_VIV2))
	{
		src2.pal = src->pal;
	}
...
BOOL DRW_DrawZOOM2_TT_Std(BMP_T *dest, int dx, int dy, BMP_T *src, int sx, int sy, int sw, int sh, int nuki, int r, int g, int b)
{
	int x, y;
	...
	else
	{
		...
		else
		{
			for (y = 0; y < sh; y++, dest_p += drev, src_p += srev)
			{
				for (x = 0; x < sw; x++, dest_p += 2, src_p++)
				{
					*(dest_p + ddx + 1) = *(dest_p + ddx) = *(dest_p + 1) =  *dest_p = rtbl[((RGB32*)src_p)->r] + gtbl[((RGB32*)src_p)->g] + btbl[((RGB32*)src_p)->b];
				}
			}
		}
	}
	...
	if (r == 0x80 && g == 0x80 && b == 0x80)
	{
		if (nuki ==  - 1)
		{
			if (MMX_Flag && !dobj->rparam)
			{
				DWORD alpha1[2] = 
				{
					(blnd) *0x00010001, (blnd) *0x00010001
				};
				DWORD alpha2[2] = 
				{
					(256-blnd) *0x00010001, (256-blnd) *0x00010001
				};
				int dwa = dw % 2;
...
BOOL DRW_DrawZOOM_TT(DRAW_OBJECT dobj)
{
	...
	if (dest->buf == NULL)
	{
		DebugPrintf("Ãè»­îIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawZOOM_TT]\n");
		return FALSE;
	}
	if (src->buf == NULL)
	{
		DebugPrintf("»­ÏñîIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawZOOM_TT]\n");
		return FALSE;
	}
	...
	if (dobj.clip != (void*) - 1)
	{
		if (!ClipRectZ(&dobj.s1x, &dobj.s1y, &dobj.s1w, &dobj.s1h,  &dobj.dx, &dobj.dy, &dobj.dw, &dobj.dh, &xx, &yy, dest->sx, dest->sy, src->sx, src->sy, dobj.clip)
			)
		{
			return 1;
		}
...
BOOL DRW_DrawZOOM_TB(DRAW_OBJECT dobj)
{
	...
	if (dest->buf == NULL)
	{
		DebugPrintf("Ãè»­îIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawZOOM_TB]\n");
		return FALSE;
	}
	if (src->buf == NULL)
	{
		DebugPrintf("»­ÏñîIÓò¤¬¤¢¤ê¤Þ¤»¤ó[DRW_DrawZOOM_TB]\n");
		return FALSE;
	}
...
BOOL DRW_DrawXLine_TT_Std(RGB32 *dest_p, BMP_T *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int r, int
						  g, int b)
{
	int ssx = 0, ssy = 0;
	int x;
...
BOOL DRW_DrawXLine_TT_Bld(RGB32 *dest_p, BMP_T *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int r, int
						  g, int b, int blnd)
{
	int ssx = 0, ssy = 0;
	int x;
...
BOOL DRW_DrawXLine_TT_Add(RGB32 *dest_p, BMP_T *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int r, int
						  g, int b)
{
	int ssx = 0, ssy = 0;
	int x;
...
BOOL DRW_DrawXLine_TT_Sub(RGB32 *dest_p, BMP_T *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int r, int
						  g, int b)
{
	int ssx = 0, ssy = 0;
	int x;
...
BOOL DRW_DrawXLine_TT_Mul(RGB32 *dest_p, BMP_T *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int r, int
						  g, int b)
{
	int ssx = 0, ssy = 0;
	int x;
...
BOOL DRW_DrawXLine_TT_Ami(RGB32 *dest_p, BMP_T *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int r, int
						  g, int b, int mesh, int dx, int dy)
{
	int ssx = 0, ssy = 0;
	int x;
	RGB32 *src_p = src->buf + sy * src->sx + sx;
...
BOOL DRW_DrawXLine_TT(RGB32 *dest_p, int dy, BMP_T *src, MIN_MAX *mi, MIN_MAX *ma, int nuki, RECT *clip, int r, int g, int b, int param, int blnd)
{
	BOOL ret = FALSE;
	int dx1, dx2, w;
	int xx, ww = ma->x - mi->x;
...
BOOL DRW_DrawXLine_TB_Std(RGB32 *dest_p, BMP_BT *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki)
{
	int ssx = 0, ssy = 0;
	int x;
...
BOOL DRW_DrawXLine_TB_Bld(RGB32 *dest_p, BMP_BT *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int blnd)
{
	int ssx = 0, ssy = 0;
	int x;
	char *src_p = src->buf + sy * src->sx + sx;
...
BOOL DRW_DrawXLine_TB_Add(RGB32 *dest_p, BMP_BT *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki)
{
	int ssx = 0, ssy = 0;
	int x;
	char *src_p = src->buf + sy * src->sx + sx;
...
BOOL DRW_DrawXLine_TB_Sub(RGB32 *dest_p, BMP_BT *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki)
{
	int ssx = 0, ssy = 0;
	int x;
	char *src_p = src->buf + sy * src->sx + sx;
...
BOOL DRW_DrawXLine_TB_Mul(RGB32 *dest_p, BMP_BT *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki)
{
	int ssx = 0, ssy = 0;
	int x;
	char *src_p = src->buf + sy * src->sx + sx;
...
BOOL DRW_DrawXLine_TB_Ami(RGB32 *dest_p, BMP_BT *src, int sx, int sy, int psx, int psy, int dsx, int dsy, int fsx, int fsy, int w, int ww, int nuki, int mesh,
						  int dx, int dy)
{
	int ssx = 0, ssy = 0;
	int x;
