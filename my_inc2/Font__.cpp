...
HFONT FNT_SetFont(LPCTSTR face, int h, int w)
{
	...
	hFont = CreateFont(h, 0, 0, 0, w, FALSE, FALSE, FALSE, 
		DEFAULT_CHARSET/*SHIFTJIS_CHARSET*/, OUT_DEFAULT_PRECIS,
		CLIP_DEFAULT_PRECIS, PROOF_QUALITY, FIXED_PITCH | FF_MODERN, face);
	if (hFont == NULL)
	{
		DebugBox(NULL, "¥Õ¥©¥ó¥ÈºîÀ®¤Ë¼ºÇÔ¤·¤Þ¤·¤¿");
	}
	return hFont;
}
...
void FNT_DrawTextBuf_T(BMP_T *dest, int dest_x, int dest_y, char *fbuf, int fsize, RECT *clip, int r, int g, int b, int
	alph, int kage, int han)
{
	int x, y;
...
void FNT_DrawTextBuf_Fx2(BMP_F *dest, int dest_x, int dest_y, char *fbuf, int fsize, RECT *clip, int r, int g, int b,
	int alph, int kage, int han)
{
	int x, y;
	RGB24 *dest_p;
	char *src_p;
...
void FNT_DrawTextBuf_Fkage(BMP_F *dest, int dest_x, int dest_y, char *fbuf, int fsize, int width, RECT *clip, int r, int
	g, int b, int alph, int han)
{
	int x, y;
	RGB24 *dest_p;
	char *src_p;
...
void FNT_DrawTextBuf_F(BMP_F *dest, int dest_x, int dest_y, char *fbuf, int fsize, RECT *clip, int r, int g, int b, int
	alph, int kage, int han)
{
	int x, y;
	RGB24 *dest_p;
...
void FNT_DrawTextBuf_H(BMP_H *dest, int dest_x, int dest_y, char *fbuf, int fsize, RECT *clip, int r, int g, int b, int
	alph, int kage, int han)
{
	int x, y;
	WORD *dest_p;
		...
		if (kage)
		{
			for (y = 0; y < h; y++, dest_p += drev, src_p += srev)
			{
				for (x = 0; x < w; x++, dest_p += 2, src_p++)
				{
					...
					{
						work = 15-(*src_p &0x0f);
						*dest_p = (WORD)(((((*dest_p &rgb16.mask_rb) *work) >> 4) &rgb16.mask_rb) | ((((*dest_p &rgb16.mask_g) *work) >> 4)
	&rgb16.mask_g));
					}
					if (*src_p &0xf0)
					{
						work = 15-(*src_p >> 4);
						*(dest_p + 1) = (WORD)(((((*(dest_p + 1) &rgb16.mask_rb) *work) >> 4) &rgb16.mask_rb) | ((((*(dest_p + 1)
	&rgb16.mask_g) *work) >> 4) &rgb16.mask_g));
					}
				}
			}
		}
		else
		{
			...
			for (y = 0; y < h; y++, dest_p += drev, src_p += srev)
			{
				for (x = 0; x < w; x++, dest_p += 2, src_p++)
				{
					...
					{
						work =  *src_p &0x0f;
						*dest_p = (WORD)(((((*dest_p &rgb16.mask_rb)*(15-work)) >> 4) &rgb16.mask_rb) | ((((*dest_p &rgb16.mask_g)*(15
	-work)) >> 4) &rgb16.mask_g)) + rgb[work];
					}
					if (*src_p &0xf0)
					{
						work =  *src_p >> 4;
						*(dest_p + 1) = (WORD)(((((*(dest_p + 1) &rgb16.mask_rb)*(15-work)) >> 4) &rgb16.mask_rb) | ((((*(dest_p + 1)
	&rgb16.mask_g)*(15-work)) >> 4) &rgb16.mask_g)) + rgb[work];
					}
				}
			}
		}
			...
			for (y = 0; y < h; y++, dest_p += drev, src_p += srev)
			{
				for (x = 0; x < w; x++, dest_p += 2, src_p++)
				{
					if (*src_p &0x0f)
					{
						...
						*dest_p = (WORD)(((((*dest_p &rgb16.mask_rb)*(16-work)) >> 4) &rgb16.mask_rb) | ((((*dest_p &rgb16.mask_g)*(16
	-work)) >> 4) &rgb16.mask_g)) + rgb[work];
					}
					if (*src_p &0xf0)
					{
						work = ((((*src_p &0xf0) + 0x10) *alph) >> 12);
						*(dest_p + 1) = (WORD)(((((*(dest_p + 1) &rgb16.mask_rb)*(16-work)) >> 4) &rgb16.mask_rb) | ((((*(dest_p + 1)
	&rgb16.mask_g)*(16-work)) >> 4) &rgb16.mask_g)) + rgb[work];
					}
				}
			}
		}
...
BOOL FNT_DrawChar(void *dest, int draw_mode, int dest_x, int dest_y, int moji, int fsize, RECT *clip, int r, int g, int
	b, int alph, int kage, int han)
{
	char *buf,  *kbuf;
	int size1, ksize1;
	int size2, ksize2;
	...
	if (i == 8)
	{
		...
		if (i == 8)
		{
			DebugPrintf("¤½¤Î¥µ¥¤¥º¤Î¥Õ¥©¥ó¥È¤ÏÆÉ¤ß¤³¤Þ¤ì¤Æ¤¤¤Þ¤»¤ó[%d]\n", fsize);
			return FALSE;
		}
	}
	switch (draw_mode)
	...
	if (no ==  - 1)
	{
		DebugPrintf("ÉÁ²è¤Ç¤­¤Ê¤¤Ê¸»ú¤¬´Þ¤Þ¤ì¤Æ¤¤¤Þ¤¹¡£\n");
		return FALSE;
	}
		...
		if (kage)
		{
			switch (draw_mode)
			{
				case 32:
					...
					break;
				case 24:
					FNT_DrawTextBuf_Fkage(bmp_f, dest_x - width + kage - 1, dest_y - width + kage - 1, kbuf, fsize, width, clip, 0, 0,
	0, alph, han);
					break;
				case 16:
					FNT_DrawTextBuf_H(bmp_h, dest_x + kage, dest_y + kage, buf, fsize, clip, r, g, b, alph, 1, han);
...
void FNT_DrawText(void *dest, int draw_mode, int dest_x, int dest_y, int pich_w, int pich_h, char *str, int fsize, RECT
	*clip, int r, int g, int b, int alph, int kage)
{
		...
		if (FontStruct[i].fsize == fsize || FontStruct[i].fsize == 0)
		{
			...
			if (PAC_LoadFile(PackFontDir, buf, &FontStruct[i].buf))
			{
				...
				if (PAC_CheckFile(PackFontDir, buf))
				{
					...
					{
						DebugPrintf("ÂÀ»ú¥Õ¥©¥ó¥È¥Õ¥¡¥¤¥ë¤ÎÆÉ¤ß¹þ¤ß¤Ë¼ºÇÔ¤·¤Þ¤·¤¿\n");
					}
				}
			...
			{
				DebugPrintf("¥Õ¥©¥ó¥È¥Õ¥¡¥¤¥ë¤ÎÆÉ¤ß¹þ¤ß¤Ë¼ºÇÔ¤·¤Þ¤·¤¿\n");
			}
