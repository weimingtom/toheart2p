...
{
	END_INITLOOP = 0,
	END_INITSTART,			//新游戏的开始
	END_LOADSTART,			//从主游戏开始画面加载进度
	END_OMAKESTART,			//おまけ模式开始
	END_MOVIE_MODE,			//电影模式
	END_GAMEEND,			//游戏结束
};	//游戏当前进行模式
...
BOOL TTL_SystemMain(void )
{
	...
	int		select = MUS_GetMouseNoEx( -1, mus_lay );	//当前高亮的菜单项
	...
	switch( TitleStep[1] )
	{
		...
		case TTL_SYS_LOAD:
			...
			case -2:		//加载存档后操作
				...
				{
					SaveWindow.load_flag = OFF;		//加载存档标志里复位
					Title.end_mode = END_LOADSTART;	//从开头加载存档模式
