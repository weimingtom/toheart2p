...
char *tone_fname[] = 
{
	NULL, "evening.amp", "night.amp", "indoor.AMP", 
};
char tone_fname2[32];
char spani_fname[32];
char *eff_fname;
...
void AVG_SetBack(int bak_no, int x, int y, int chg_type, int char_disp, int cg_flag, int fd_max, int vague)
{
	...
	else
	{
		if (BackStruct.sp_flag)
		{
			...
			DSP_CopySprite(BMP_SPBACKHALF, BMP_SPBACK, NULL, 256, BackStruct.r *Avg.half_tone / 128, BackStruct.g *Avg.half_tone / 128, BackStruct.b *Avg.half_tone /
	128);
		...
		else
		{
			...
			else
			{
				...
				if (BackStruct.br_type &0x20)
				{
					...
					DSP_SetGraphPosPoly(GRP_WORK + 20, BackStruct.rx + COS(rate) / 4, BackStruct.ry + SIN(rate) / 4, BackStruct.rx + COS(rate + 64) / 4, BackStruct.ry + SIN
	(rate + 64) / 4, BackStruct.rx + COS(rate + 192) / 4, BackStruct.ry + SIN(rate + 192) / 4, BackStruct.rx + COS(rate + 128) / 4, BackStruct.ry + SIN(rate +
	128) / 4, 0, 0, 799, 0, 0, 599, 799, 599);
				}
			}
		}
	}
...
void AVG_SetBackScrollPoly(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int frame, int type)
{
	BackStruct.sc_flag = 1;
	BackStruct.sx = x1;
	BackStruct.sy = y1;
		...
		if (sc)
		{
			switch (BackStruct.sc_type / 3)
			{
				...
				case 1:
					...
					break;
				case 2:
					AVG_SetBackPosPoly(BackStruct.sx, BackStruct.sy, BackStruct.sx2, BackStruct.sy2, BackStruct.sx3, BackStruct.sy3, BackStruct.sx4, BackStruct.sy4);
					break;
			}
		}
		else
		{
			switch (BackStruct.sc_type / 3)
			{
				...
				case 1:
					...
					break;
				case 2:
					AVG_SetBackPosPoly(BackStruct.x, BackStruct.y, BackStruct.x2, BackStruct.y2, BackStruct.x3, BackStruct.y3, BackStruct.x4, BackStruct.y4);
					break;
			}
		}
...
BOOL AVG_SetCalender(int _month, int _days)
{
	...
	switch (step)
	{
		...
		case 1:
			...
			for (i = 0; i < 5; i++)
			{
				DSP_SetGraphParam(GRP_MAP + i, DRW_BLD(count *16));
			}
			if (count >= 16)
			{
				for (i = 0; i < 5; i++)
				{
					DSP_SetGraphParam(GRP_MAP + i, DRW_NML);
				}
				step = 2;
			}
			break;
		...
		case 3:
			...
			for (i = 0; i < 5; i++)
			{
				DSP_SetGraphFade(GRP_MAP + i, 128-count * 8);
			}
			if (count >= 16)
			{
				step = 4;
...
{
	8 *60+43, 9 *60+5, 9 *60+25, 9 *60+35, 10 *60+0, 10 *60+20, 10 *60+30, 10 *60+55, 11 *60+15, 11 *60+25, 11 *60+50, 12 *60+10, 12 *60+35, 13 *60+0, 13 *60+25,
	13 *60+45, 13 *60+55, 14 *60+20, 14 *60+40, 14 *60+50, 
};
