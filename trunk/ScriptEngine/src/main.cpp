...
static int ScriptFlag = 1; //当前是否是AVG解析模式
...
int MainStep = MAIN_INIT;		//当前游戏处理状态
int NextMainStep = MAIN_INIT;	//下一帧游戏处理状态
int GameStep = GAME_TITLE;		//当前游戏的位置状态
DEBUG_TEXT Debug = 
{
	OFF, ON
};
...
//-------------------------------------------------------------------------
//函数名：			MAIN_SetScriptFlag
//功能：			设定当前游戏模式
//参数flag：ON:1　　AVG解析模式
//参数flag：OFF:0　 AVG非解析模式，播放OP，开始画面选择等等
...
//-------------------------------------------------------------------------
//函数名：			MAIN_GetScriptFlag
//功能：			得到当前游戏模式
//返回值：			
//		ON:1　　AVG解析模式
//		OFF:0　 AVG非解析模式，播放OP，开始画面选择等等
...
//-------------------------------------------------------------------------
//函数名：	MAIN_GameControl
//功能：	主游戏控制
//
//输入参数：			
//			类型		参数名			意义
//			int			script			当前是否是AVG解析模式
//返回值：
//		返回下一帧游戏的状态
int MAIN_GameControl(int script)
{
	...
	{
		default:
		case GAME_DEMO:		//播放游戏DEMO
			DEM_System();
			break;
		case GAME_TITLE:	//游戏开场画面中处理
			GameStep = TTL_System();
			break;
		case GAME_OPENNING:	//OP
			break;
		case GAME_AVG:		//AVG解析系统处理
			GameStep = AVG_System(script);
			break;
		case GAME_ENDING:	//ENDING
			break;
		case GAME_OMAKE:	//看图模式
			break;
	}
	...
	switch (MainStep)
	{
		...
		case MAIN_GAME:
			...
			if (Debug.flag)
			{
				...
				DRW_DrawDmoji(dest, 8, y, work, 0, MainWindow.draw_mode2);
				y += 8;
				...
				if (Debug.game)
				{
					DRW_DrawDmoji(dest, 8, y, "*GAME FLAG", 0, MainWindow.draw_mode2);
					y += 8;
					...
					for (i = 0; i < 256; i += 16)
					{
						wsprintf(work, "%02d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d", i / 16, ESC_GameFlagBuf[i], ESC_GameFlagBuf[i + 1],
	ESC_GameFlagBuf[i + 2], ESC_GameFlagBuf[i + 3], ESC_GameFlagBuf[i + 4], ESC_GameFlagBuf[i + 5], ESC_GameFlagBuf[i + 6], ESC_GameFlagBuf[i + 7],
	ESC_GameFlagBuf[i + 8], ESC_GameFlagBuf[i + 9], ESC_GameFlagBuf[i + 10], ESC_GameFlagBuf[i + 11], ESC_GameFlagBuf[i + 12], ESC_GameFlagBuf[i + 13],
	ESC_GameFlagBuf[i + 14], ESC_GameFlagBuf[i + 15]);
						DRW_DrawDmoji(dest, 8, y, work, 0, MainWindow.draw_mode2);
						y += 8;
					}
				}
				...
				if (Debug.scenario)
				{
					DRW_DrawDmoji(dest, 8, y, "*FLAG", 0, MainWindow.draw_mode2);
					y += 8;
					if (Debug.scenario == 1)
					{
						wsprintf(work, "    00  01  02  03  04  05  06  07  08  09  10  11  12  13  14  15");
						...
						for (i = 0; i < 256; i += 16)
						{
							wsprintf(work, "%02d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d", i / 16, ESC_FlagBuf[i], ESC_FlagBuf[i + 1], ESC_FlagBuf[i + 2],
	ESC_FlagBuf[i + 3], ESC_FlagBuf[i + 4], ESC_FlagBuf[i + 5], ESC_FlagBuf[i + 6], ESC_FlagBuf[i + 7], ESC_FlagBuf[i + 8], ESC_FlagBuf[i + 9], ESC_FlagBuf[i +
	10], ESC_FlagBuf[i + 11], ESC_FlagBuf[i + 12], ESC_FlagBuf[i + 13], ESC_FlagBuf[i + 14], ESC_FlagBuf[i + 15]);
							DRW_DrawDmoji(dest, 8, y, work, 0, MainWindow.draw_mode2);
							y += 8;
						}
					}
					else
					{
						...
						for (i = 0; i < 256; i += 16)
						{
							wsprintf(work, "%x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x", i / 16, ESC_FlagBuf[i], ESC_FlagBuf[i + 1],
	ESC_FlagBuf[i + 2], ESC_FlagBuf[i + 3], ESC_FlagBuf[i + 4], ESC_FlagBuf[i + 5], ESC_FlagBuf[i + 6], ESC_FlagBuf[i + 7], ESC_FlagBuf[i + 8], ESC_FlagBuf[i +
	9], ESC_FlagBuf[i + 10], ESC_FlagBuf[i + 11], ESC_FlagBuf[i + 12], ESC_FlagBuf[i + 13], ESC_FlagBuf[i + 14], ESC_FlagBuf[i + 15]);
							DRW_DrawDmoji(dest, 8, y, work, 0, MainWindow.draw_mode2);
							y += 8;
						}
					}
				...
				if (Debug.reg)
				{
					DRW_DrawDmoji(dest, 8, y, "*REGISTER", 0, MainWindow.draw_mode2);
					y += 8;
					if (Debug.reg == 1)
					{
						...
						for (i = 0; i < 5; i++)
						{
							wsprintf(work, "%02d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d", i, EXEC_LangInfo->reg[0+i * 10], EXEC_LangInfo->reg[1+i * 10], EXEC_LangInfo->reg[2+i *
	10], EXEC_LangInfo->reg[3+i * 10], EXEC_LangInfo->reg[4+i * 10], EXEC_LangInfo->reg[5+i * 10], EXEC_LangInfo->reg[6+i * 10], EXEC_LangInfo->reg[7+i * 10],
	EXEC_LangInfo->reg[8+i * 10], EXEC_LangInfo->reg[9+i * 10]);
							DRW_DrawDmoji(dest, 8, y, work, 0, MainWindow.draw_mode2);
							y += 8;
						}
					}
					else
					{
						...
						for (i = 0; i < 5; i++)
						{
							wsprintf(work, "%x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x", i, EXEC_LangInfo->reg[0+i * 10], EXEC_LangInfo->reg[1+i * 10], EXEC_LangInfo
	->reg[2+i * 10], EXEC_LangInfo->reg[3+i * 10], EXEC_LangInfo->reg[4+i * 10], EXEC_LangInfo->reg[5+i * 10], EXEC_LangInfo->reg[6+i * 10], EXEC_LangInfo
	->reg[7+i * 10], EXEC_LangInfo->reg[8+i * 10], EXEC_LangInfo->reg[9+i * 10]);
							DRW_DrawDmoji(dest, 8, y, work, 0, MainWindow.draw_mode2);
							y += 8;
						}
					}
