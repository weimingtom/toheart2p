...
static int BmpSetting;		//BMP集合结构体中存放的索引号
static int GrpSetting;		//BMP集合结构体中存放的索引号
static int TxtSetting;		//BMP集合结构体中存放的索引号

static int GrpBack;			//BMP集合结构体中存放的索引号
static int TxtBack;			//BMP集合结构体中存放的索引号

static int MouseLayer;		//鼠标控制的层号
static int MouseLayerBack;	//鼠标控制的上一层的层号
...
enum
{
	SAV_NOT, SAV_OPEN, SAV_CLOSE, SAV_LOAD_OPEN, SAV_LOAD_CLOSE, SAV_LOAD_CHECK, SAV_NORMAL, 
};
...
void GWIN_RenewSaveLoadWindow(void)
{
	...
	for (i = 0; i < SAVE_PAGE; i++)
	{
...

		{
			...
			char *_time_tbl[8] = 
			{
				"朝", "休み時間", "昼休み", "授業中", "授業終了後", "放課後", "夜", "次の日へ"
			};
...
//-------------------------------------------------------------------------
//函数名：	GWIN_SetSaveLoadWindow
//功能：	读档存档窗口加载初始化操作
//
//输入参数：			
//			类型		参数名			意义
//			int			type			模式：	W_SAVE_MODE		0
//												SW_LOAD_MODE	1	(TitleLoad = 1)
//												SW_LOAD_MODE2	2	(TitleLoad = 0)
//			int			bmp_set			BMP集合结构体中所对应存放的索引号，这里等于bmp_set＝40
//			int			grp_set			BMP集合结构体中所对应存放的索引号，这里等于grp_set＝40
//			int			txt_set			BMP集合结构体中所对应存放的索引号，这里等于grp_set＝40
//			int			grp_bak			BMP集合结构体中所对应存放的索引号，这里等于grp_set＝-1
//			int			mus_lay			鼠标控制的当前层
//			int			mus_bak
//返回值：
//		无
...
//-------------------------------------------------------------------------
//函数名：	GWIN_ControlSaveLoadWindow
//功能：	控制读档存档窗口
//
//输入参数：无	
//			类型		参数名			意义
...
int GWIN_ControlSaveLoadWindow(void)
{
	...
	int select = MUS_GetMouseNoEx( - 1, MouseLayer);		//当前高亮的对象
	int click = GameKey.click;		//左键
	int cansel = GameKey.cansel;	//右键
	...
	if (SaveWindow.flag)
	{
		switch (SaveWindow.mode)
		{
			...
			case SAV_LOAD_CLOSE:
				...
				break;
			case SAV_LOAD_CHECK:		//【是否加载此进度】提示信息窗口显示状态
				rate2 = 256;
				...
				if (!SaveWindow.load_flag)
				{
					...
					if (click)		//点了其中的一个按钮
					{
						if (select == 0)		//点了“はい”
						{
							...
							if (SaveWindow.type == SW_SAVE_MODE)		//保存存档模式
							{
								...
								SaveWindow.mode = SAV_CLOSE;			//设定保存模式标志
								SaveWindow.cnt = 0;
							}
							else										//加载存档模式
							{
								SaveWindow.flag = OFF;
								SaveWindow.load_flag = ON;				//设定加载模式标志
			...
			case SAV_NORMAL:			//SAVE状态时控制
				...
				if (cansel)	//取消当前读取／保存窗口，返回上一个画面
				{
					AVG_PlaySE3(9107, 255);
					SaveWindow.mode = SAV_CLOSE;//设定窗口关闭状态
					SaveWindow.cnt = 0;
				}
				break;
		}
...
enum
{
	SIDE_NOT, SIDE_OPEN, SIDE_CLOSE, SIDE_NORMAL, 
};
...
void GWIN_SetSideBarWindowEx(int bmp_set, int grp_set, int txt_set, int mus_lay, int mus_bak)
{
	...
	for (i = 0; i < 10; i++)
	{
		for (j = 0; j < 5; j++)
		{
			...
			DSP_SetGraphPos(GrpSetting + 10+i * 5+j, OptinOfset.x + OptionPosition[i][j].x, OptinOfset.y + OptionPosition[i][j].y, OptionPosition[i][j].x,
	OptionPosition[i][j].y, OptionPosition[i][j].w, OptionPosition[i][j].h);

			MUS_SetMouseRect(MouseLayer, i *5+j, OptinOfset.x + OptionPosition[i][j].x, OptinOfset.y + OptionPosition[i][j].y, OptionPosition[i][j].w,
	OptionPosition[i][j].h, (OptionPosition[i][j].w) ? ON : OFF);
		}
	}
		...
		switch (SideBarWindow.mode)
		{
			...
			case SIDE_NORMAL:
				...
				if (select !=  - 1)
				{
					...
					{
						MainWindow.draw_flag = 1;
						if (!((select / 5 == 3 && (select % 5 == 0 || select % 5 == 1)) || (select / 5 == 4 && (select % 5 == 0 || select % 5 == 1)) || (select / 5 == 5 && 
	(select % 5 == 0 || select % 5 == 1)) || (select / 5 == 0)))
						{
							AVG_PlaySE3(9104, 255);
						}
						if (select < 50)
...
enum
{
	SET_NOT, SET_OPEN, SET_CLOSE, SET_NORMAL, 
};
...
static void GWIN_InitSettingBar(void)
{
	...
	SettingWindow.backup[2] = SettingWindow.select[2] = (short)Avg.win_bld - 1;
...
BOOL GWIN_ControlSettingWindow(void)
{
	...
	if (SettingWindow.flag)
	{
		...
		if (SettingWindow.mode == SET_CLOSE || SettingWindow.mode == SET_OPEN)
		{
			...
			if ((SettingWindow.cnt >= cmax / 2 && SettingWindow.mode == SET_OPEN) || (SettingWindow.cnt < cmax / 2 && SettingWindow.mode == SET_CLOSE))
			{
