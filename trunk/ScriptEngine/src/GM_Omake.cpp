...
int OMK_ControlCgMode(void)
{
	enum
	{
		CG_INIT, CG_TERM, CG_FADEIN, CG_FADEOUT, CG_MAIN, CG_PAGE_FADE, CG_VIEW_OPEN, CG_VIEW_CLOSE, CG_VIEW_MAIN, CG_VIEW_PAGE, CG_VIEW_SCROOL, 
	};
	BOOL ret = 0;
	int bmp_bpp = BPP(MainWindow.draw_mode2);
	...
	switch (step)
	{
		...
		case CG_VIEW_OPEN:
			...
			if (count < fmax)
			{
				...
				if (sy == 600)
				{
					DSP_SetGraphZoom(CGGP_SYS + 10, (54+177 *(cg % 4))*(256-rate) / 256, (77+130 *(cg / 4))*(256-rate) / 256, 160 *(256-rate) / 256+800 * rate / 256, 120 *
	(256-rate) / 256+600 * rate / 256);
				}
				else
				{
					DSP_SetGraphPosZoom(CGGP_SYS + 10, (54+177 *(cg % 4))*(256-rate) / 256, (77+130 *(cg / 4))*(256-rate) / 256, 160 *(256-rate) / 256+800 * rate / 256, 120 *
	(256-rate) / 256+600 * rate / 256, 0, 600, 800, 600);
				}
			}
			else
			{
...
int OMK_ControlReplayMode(void)
{
	enum
	{
		REPLAY_INIT, REPLAY_TERM, REPLAY_FADEIN, REPLAY_FADEOUT, REPLAY_MAIN, 
	};
	BOOL ret = 0;
	int bmp_bpp = BPP(MainWindow.draw_mode2);
...
int OMK_ControlSoundMode(void)
{
	enum
	{
		SOUND_INIT, SOUND_TERM, SOUND_FADEIN, SOUND_FADEOUT, SOUND_MAIN, 
	};
	BOOL ret = 0;
	int bmp_bpp = BPP(MainWindow.draw_mode2);
