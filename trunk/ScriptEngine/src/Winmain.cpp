...
static LRESULT CALLBACK WndProc( HWND hwnd, UINT wmes, UINT wparam, LONG lparam)
{
	...
	switch( wmes) 
	{
		...
		case WM_TIMER:
			...
			break;
		case WM_ACTIVATE:		//当前窗口是激活的
			MainWindow.active = LOWORD(wparam);
			KEY_RenewKeybord( MainWindow.active );						//重设键盘输入
			MUS_RenewMouse( MainWindow.hwnd, MainWindow.active, 0 );	//重设鼠标输入
			break;
		case WM_COMMAND:
			MUS_SetClickThrough();
