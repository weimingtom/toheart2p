...
static int CharLocateTable[9] = 
{
	- 160 * 80 / 64, 0, 160 *80 / 64,  - 192 * 80 / 64, 192 *80 / 64,  - 300 * 80 / 64, 300 *80 / 64,  - 400 * 80 / 64, 400 *80 / 64
};
#define CHAR_DY	0
...
static void SetCharPos(int index, int x)
{
	...
	{
		if (!((90 <= CharStruct[index].pose && CharStruct[index].pose < 100) || (190 <= CharStruct[index].pose && CharStruct[index].pose < 200)))
		{
...
void SetCharPosShake(int shx, int shy, int disp)
{
	...
	for (i = 0; i < MAX_CHAR; i++)
	{
		if (CharStruct[i].flag)
		{
			...
			{
				if (!((90 <= CharStruct[i].pose && CharStruct[i].pose < 100) || (190 <= CharStruct[i].pose && CharStruct[i].pose < 200)))
				{
...
void AVG_LoadChar(int index, int char_no, int pose, int tone_no, int in_type)
{
	...
	DSP_CopyBmp2(BMP_CHARHALF + index * 2+CharStruct[index].grp, BMP_CHAR + index * 2+CharStruct[index].grp, NULL, 256, BackStruct.r *AVG_GetHalfToneFade() / 128,
		BackStruct.g *AVG_GetHalfToneFade() / 128, BackStruct.b *AVG_GetHalfToneFade() / 128);
...
void AVG_SetChar(int char_no, int pose, int locate, int layer, int in_type, int bright, int alph, int frame)
{
	...
	if (index != MAX_CHAR)
	{
		...
		if (CharStruct[index].cno == char_no && CharStruct[index].pose == pose && CharStruct[index].loc1 == locate && CharStruct[index].fade1 == bright &&
			CharStruct[index].layer == layer && CharStruct[index].alph1 == alph)
		{
			return ;
		}
	}
...
void AVG_SetCharPose(int char_no, int pose, int in_type, int frame)
{
	...
	if (index != MAX_CHAR)
	{
		if (in_type == CHAR_TYPE_WAIT)
		{
			AVG_SetChar(char_no, pose, CharStruct[index].loc1, CharStruct[index].layer, CHAR_TYPE_WAIT, CharStruct[index].fade1, CharStruct[index].alph1, frame);
		}
		else
		{
			if (CharStruct[index].pose == pose)
		...
		if (CharStruct[i].flag)
		{
			...
			case CHAR_COND_IN:
				if (max)
				{
					...
					{
					case CHAR_TYPE_MOVE:
					case CHAR_TYPE_MOVE + 1: cnt = max - cnt;
						cnt = cnt * cnt * cnt;
						max = max * max * max;
						cnt = max - cnt;
						...
						rate = (int)(INT64(CHAR_LOCATE(CharStruct[i].loc1) - locate) *cnt / max);
			...
			case CHAR_COND_LOCATE:
				if (max)
				{
					...
					max = max * max * max;
					cnt = max - cnt;
					rate = (int)(INT64(CHAR_LOCATE(CharStruct[i].loc1) - CHAR_LOCATE(CharStruct[i].loc2)) *cnt / max);
