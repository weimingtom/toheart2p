...
#define DOS_FONT	20	
#define SYS_FONT	34
#define RUBI_FONT	16			//ルビフォント注音字体
#define CHIP_FONT	24	
...
typedef struct
{
	...
	long		msg_page;		
	long		msg_wait;		
	long		msg_cut;			//当前是否在快速跳过信息
	long		msg_cut_mode;		//快速跳过信息标志
	long		msg_font;			
	long		half_tone;		
...
typedef struct
{
	...
	int		mes_cut;			//快速跳过消息标志位
	int		mes_cut_mode;	
	int		diswin;				//显示下一句消息标志位
	
	int		u,d,l,r;	

	int		end;	
	int		home;	
	int		pup;				//向前翻页，保存读取存档窗口
	int		pdown;				//向后翻页，保存读取存档窗口
