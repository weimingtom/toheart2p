...
AVG_STRUCT Avg = 
{
	60, MES_NORMAL, DRAW_HIGHEND, 0, OFF, ON, WIN_BLEND, __RESERV, 

	1, 1, OFF, MES_NORMAL, OFF, OFF, 0, 64, BGM_PCM, ON, ON, ON, 0, PCM_44_16, ON, 128+64-16, 256, 256, DEMO_NOT, 0, 30, OFF, 0, 1, 0, 0, 0, ON, OFF, OFF, 120,
	240, 
};
int CharVoiceFlag[CHAR_VOICE_MAX] = 
{
...
enum
{
	AVG_INIT = 0, AVG_GAME, AVG_MAP, AVG_CALENDER, AVG_CONFIG, AVG_SAVE, AVG_LOAD, AVG_SETTING, AVG_END, 
};
...
void AVG_SetWeather(int wno, int wind, int speed, int amount, int no_reset)
{
	...
	switch (wno)
	{
		...
		case WEATH_SAKURA:
			...
			{
				Weather.twind = wind;
				Weather.wind = (float)Weather.twind;
...
enum
{
	MUSIC_STOP = 0, MUSIC_PLAY, MUSIC_PAUSE, MUSIC_FADEIN, MUSIC_FADEOUT, MUSIC_FADEVOL, 
};
...
void AVG_SetDirectVoumeVoiceAll(int voice_vol)
{
	...
	for (i = 0; i < VOICE_MAX; i++)
	{
		if (VoiceStruct[i].handle !=  - 1)
		{
			vol = (int)((float)VoiceStruct[i].vol *voice_vol * VoiceVolumeScript / (256 *256));
...
void AVG_SizukuPlayVoice(int vc_no, int vno, int char_no, int ex_no, int vol, int loop, int goin)
{
	...
	vol2 = (int)((float)VoiceStruct[vc_no].vol *Avg.voice_vol * VoiceVolumeScript / (256 *256));
...
void AVG_ControlSound(void)
{
	...
	for (i = 0; i < VOICE_MAX; i++)
	{
		if (VoiceStruct[i].handle !=  - 1)
		{
			...
			vol = (int)((float)VoiceStruct[i].vol *Avg.voice_vol * VoiceVolumeScript / (256 *256));
			lpSoundDS->SetVolume(VoiceStruct[i].handle, LIM(vol, 0, 255), FALSE);
		}
	}
...
enum
{
	CNF_NOT, CNF_OPEN, CNF_CLOSE, CNF_NORMAL, CNF_DIS_TEXT, CNF_ANOTHER, 

	CNF_NEXT_OPEN, CNF_NEXT_CLOSE, 
...
enum
{
	CNF_NEXT_SAVE, CNF_NEXT_LOAD, CNF_NEXT_SIDE, 
};
...
int AVG_ControlConfigWindow(void)
{
	...
	enum
	{
		CNF_CHK_IN = 0, CNF_CHK_OUT, CNF_CHK_MAIN, 
	};
...
enum
{
	LOAD_NOT = 0, LOAD_FADEOUT, LOAD_WORK, LOAD_FADEIN, 
};

static int LoadNo = 0;		//存档的索引号
...
enum
{
	GOTILE_NOT, GOTILE_FADEOUT, GOTILE_END, 
};
int GotoTitle = 0;
...
BOOL AVG_ConfigCheck(void)
{
	...
	bk_flag = !(BackStruct.cs_flag || BackStruct.sc_flag || BackStruct.fd_flag || BackStruct.br_flag || AVG_WaitShake());
...
void AVG_ControlSystem2(void)
{
	...
	if (GameKey.cansel || GameKey.click || GameKey.diswin || GameKey.home || GameKey.end || GameKey.pdown || GameKey.pup || Avg.demo)
	{
		Avg.msg_cut = OFF;
		Avg.msg_cut_mode = OFF;
	}
...
char *MapEventCharName[15] = 
{
	"( 0)不正　　", "( 1)このみ　", "( 2)愛佳　　", "( 3)環　　　", "( 4)花梨　　", "( 5)珊瑚　　", "( 6)瑠璃　　", "( 7)由真　　", "( 8)ルーシー", "( 9)優希　　",
	"(10)珊瑚瑠璃", "(11)ハテナ　", "(12)このみ環", "(13)雄二　　", "(14)ささら　"
};

char *MapEventPosName[22] = 
{
	"( 0)自宅　　", "( 1)商店街　", "( 2)ゲーセン", "( 3)公園　　", "( 4)中学校　", "( 5)坂道　　", "( 6)校門前　", "( 7)校庭　　", "( 8)裏庭　　", "( 9)駐輪場　",
	"(10)下駄箱　", "(11)体育館　", "(12)書庫　　", "(13)図書室　", "(14)視聴覚室", "(15)廊下１Ｆ", "(16)廊下２Ｆ", "(17)教室１Ｆ", "(18)教室２Ｆ", 
	"(19)廊下３Ｆ", "(20)教室３Ｆ", "(21)生徒会室"
};

char *MapEventCharType[4] = 
{
	"(0)制服　　", "(1)私服　　", "(2)体操服　", "(3)中学制服"
};
...
//输入参数：			
//			类型		参数名			意义
//			char*		str				要加载游戏脚本的文件名
//返回值：
//			int			ret				加载成功：1；加载失败：0
	...
	if (script == SCCODE_NOOPR && MAIN_GetScriptFlag() != 0)
	{
		...
		switch (_times)
		{
			...
			case 5:
				...
				break;
			case 6:	//晚上
				wsprintf(buf, "EV_%02d%02dNIGHT.sdt", _month, _days);
				break;
		}
