...
lx_sse2_aps:
		...
		if(size-size2*32) 
			CopyMemory( (char*)(dst)+size2*32, (char*)(src)+size2*32, size-size2*32 );
	}
	else if(SSE_FP_Flag )
	{
		if(size2)
...
#ifdef _DEBUG
	MessageBox2( hwnd, MB_OK|MB_SETFOREGROUND, "セック ", buf );
#else
	MessageBox2( hwnd, MB_OK|MB_SETFOREGROUND, "セック ", buf );
...
static CAL_LIST *factor( void )
{
	...
	{
		CalP+=2;
		while( isdigit(CalStr[CalP]) || ('a'<=CalStr[CalP] && CalStr[CalP]<='f') ||
...
//			char *		dst				目标文件名
//			char *		src				源文件名
//			char *		attr			希望变成的文件扩展名
//返回值：
//			int			src＋1			返回目标文件名的地址
...
//-------------------------------------------------------------------------
//函数名：	STD_WriteFilePos
//功能：	从指定位置开始写入数据到硬盘
//
//输入参数：			
//			类型		参数名			意义
//			char *		fname			存档文件名，例如：save_00.sav
//			char *		src				要写入的数据
//			DWORD		pos				要读取文件的位置
//			DWORD		size			要读取的字节数
//返回值：
...
// ---------------------------------------------------------------------------
//	函数名：ReadIniFileAmount
//	iniファイルから数値(long)を読み取る，从INI文件中读取指定的数据
//	参数		
//			类型			名称				意义
//			char *			vname				要读取的键名
//			int				def					读不到时的默认值
//			char *			sec_name			要读取的主APP键名
//			char *			ini_fname			INI文件的文件名
//返回值	int									读取出的整数
// ---------------------------------------------------------------------------
...
char * Digit2Str( int digit, int zero, int lr )
{
	...
	{
		if( zero )
			wsprintf( str, "%016d", digit );
		else	
			wsprintf( str, "%16d", digit );
	}
	else
...
char * Digit2Str2( int digit, int zero, int lr )
{
	...
	{
		if( zero )	
			wsprintf( str, "%02d", digit );
		else		
			wsprintf( str, "%2d", digit );
	}
	else
		wsprintf( str, "%d", digit );
...
void STD_CheckCPU( void )
{
	...
	DebugPrintf("\n");
	DebugPrintf("CPUID       %d\n", CPUID_Flag);		//CPUIDサポートの??殪
	DebugPrintf("CPUIDMax    %x\n", CPUIDMax);		//CPUID侏慳機能のサポート最大値
	DebugPrintf("CPUIDExtMax %x\n", CPUIDExtMax);	//CPUID拡張機能のサポート最大値

	DebugPrintf("FPU    %d\n", FPU_Flag);				//浮動小数点演算器の??殪
	DebugPrintf("TSC    %d\n", TSC_Flag);				//RDTSC命令の??殪
	DebugPrintf("MSR    %d\n", MSR_Flag);				//RDMSR,WRMSR命令の??殪
	DebugPrintf("CMOV   %d\n", CMOV_Flag);				//CMOV命令の??殪
	DebugPrintf("MMX    %d\n", MMX_Flag);				//MMX命令の??殪
	DebugPrintf("3DNow! %d\n", _3DNow_Flag);			//3DNow!命令の??殪

	DebugPrintf("SSE_INT    %d\n", SSE_INT_Flag);		//SSE integer(=MMX拡張)命令の??殪
	DebugPrintf("SSE_FP     %d\n", SSE_FP_Flag);		//SSE floating-point命令の??殪
	DebugPrintf("3DNow! Ext %d\n", _3DNowExt_Flag);		//3DNow!拡張命令の??殪
	DebugPrintf("FCMOV      %d\n", FPU_Flag & CMOV_Flag);//FCMOV命令の??殪
	/*
	DWORD			cpu_id = 0;
	DWORD			cpu_model = 0;
	DWORD			cpu_family = 0;
