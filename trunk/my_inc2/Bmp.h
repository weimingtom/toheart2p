...
typedef struct
{
	int flag;		//当前BMP结构体是否是有效数据
	int bmp_bit;	//8位色时0；24位色时：2；32位色时：3　　
	int alp_flag;	//Alpha通道Flag

	POINT pos;		//图片的位置
	POINT size;		//BMP图片的宽度和高度
...
extern int BMP_MakeAlphaFF2T(char *bfname, char *afname, char *tfname, int alp_flag, int alp_end, int alp250, int
CutSave);
...
#define BMP_BLD_H(D,A)			(WORD)((((((D)&rgb16.mask_rb)*(A))>>5)&rgb16.mask_rb)|(((((D)&rgb16.mask_g)*(A))>>5) \
&rgb16.mask_g))
