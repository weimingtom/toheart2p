	...
	//创建一个只读的文件指针
	#define CREATE_READ_FILE(FNAME)		CreateFile( FNAME, GENERIC_READ , FILE_SHARE_READ, NULL, OPEN_EXISTING,\
	...
	#define CREATE_READ_FILE2(FNAME,FLAG)		CreateFile( FNAME, GENERIC_READ , FILE_SHARE_READ, NULL, OPEN_EXISTING, FLAG,\
	NULL)
	...
	extern int ReadIniFileStr(char *vname, char *str, char *def, int size, char *sec_name = "DEFAULT", char *ini_fname =
	NULL);
