...
void MOV_DebugBox(HWND hwnd, char *mes, ...)
{
	...
	MessageBox(hwnd, buf, "æØ∏Ê", MB_OK | MB_SETFOREGROUND);
}
...
int MOV_OpenFile(char *fname, int w, int h, int bit)
{
	...
	if (ret ==  - 1)
	{
		MOV_DebugBox(NULL, "AVIÉXÉgÉäÅ[ÉÄÇ™ëΩâﬂÇ¨Ç‹Ç∑[4Ç¬Ç‹Ç≈]");
		return  - 1;
	}
	...
	switch (hResult)
	{
		...
		case AVIERR_BADFORMAT:
			MOV_DebugBox(NULL, "ÉtÉ@ÉCÉãÇ™îjëπÇµÇƒÇ¢ÇÈÇ©ÅACodecÇ™ÉCÉìÉXÉgÅ[ÉãÇ≥ÇÍÇƒÇ¢Ç»Ç¢ÇΩÇﬂÅAì«Ç›éÊÇËÇ≈Ç´Ç‹ÇπÇÒÇ≈ÇµÇΩÅB");
			return  - 1;
		case AVIERR_MEMORY:
			MOV_DebugBox(NULL, "ÉÅÉÇÉäïsë´ÇÃÇΩÇﬂÅAÉtÉ@ÉCÉãÇäJÇØÇ‹ÇπÇÒÇ≈ÇµÇΩÅB");
			return  - 1;
		case AVIERR_FILEREAD:
			MOV_DebugBox(NULL, "ÉtÉ@ÉCÉãÇÃì«Ç›éÊÇËíÜÇ…ÉfÉBÉXÉN ÉGÉâÅ[Ç™î≠ê∂ÇµÇ‹ÇµÇΩÅB");
			return  - 1;
		case AVIERR_FILEOPEN:
			MOV_DebugBox(NULL, "ÉtÉ@ÉCÉãÇÃÉIÅ[ÉvÉìíÜÇ…ÉfÉBÉXÉN ÉGÉâÅ[Ç™î≠ê∂ÇµÇ‹ÇµÇΩÅB");
			return  - 1;
		case REGDB_E_CLASSNOTREG:
			MOV_DebugBox(NULL, "AVIFileOpen ä÷êîÇ≈éwíËÇ≥ÇÍÇΩÉtÉ@ÉCÉãÉ^ÉCÉvÇéùÇ¬ÉnÉìÉhÉâÇ™ÅAÉåÉWÉXÉgÉäÇ…å©Ç¬Ç©ÇËÇ‹ÇπÇÒÇ≈ÇµÇΩ ");
			return  - 1;
		case E_NOINTERFACE:
		default:
			MOV_DebugBox(NULL, "âΩÇÁÇ©ÇÃå¥àˆÇ…ÇÊÇË AVIFileOpenä÷êîÇ™é∏îsÇµÇ‹ÇµÇΩ[%08x] ", hResult);
			return  - 1;
	}
	...
	if (hResult)
	{
		MOV_DebugBox(NULL, "AVIèÓïÒÇÃéÊìæÇ…é∏îsÇµÇ‹ÇµÇΩ()");
		return  - 1;
	}
	...
	for (i = 0; i < (int)FileInfo.dwStreams; i++)
	{
		...
		{
			as->bUseAudio = FALSE;
			OutputDebugString("AVIâπê∫ÇÕñ¢ÉTÉ|Å[ÉgÇ≈Ç∑\n");
			break;
	...
	{
		default:
			case  - 1: MOV_DebugBox(NULL, 
	"AVIÇ™ï`âÊÇ≈Ç´Ç‹ÇπÇÒÅB\nCodecÇ™ÉCÉìÉXÉgÅ[ÉãÇ≥ÇÍÇƒÇ¢Ç»Ç¢Ç∆évÇÌÇÍÇ‹Ç∑ÅB\nCDì‡ÇÃ[iv5setup.exe]Çé¿çsÇµÇƒÅAÉCÉìÉXÉgÅ[ÉãçÏã∆ÇçsÇ¡Çƒâ∫Ç≥Ç¢ÅB");
			as->bUseVideo = FALSE;
			break;
		case 0:
			as->bUseVideo = TRUE;
			break;
		case 1:
			OutputDebugString("éwíËÉtÉHÅ[É}ÉbÉgÇ≈ï`âÊÇ≈Ç´Ç‹ÇπÇÒÅBòÀú ÉtÉHÅ[É}ÉbÉgÇ≈çƒê∂ÇµÇ‹Ç∑ÅB\n");
...
int MOV_SetVideoBufFormat(int mov_no, int w, int h, int bit, BITMAPINFOHEADER *bi)
{
	...
	if ((as->bi.biBitCount == bit || 0 == bit) && (as->bi.biWidth == w || 0 == w) && (as->bi.biHeight == h || 0 == h))
	{
		*bi = as->bi;
		return 2;
	}
