...
typedef struct
{
	...
	short wheel;
	short wheel_get;		//鼠标滚轮操作状态

	short no;
	int interval;			//无用
	char lno;				//鼠标当前控制的图层索引号
	char flag;
...
//			类型		参数名				意义
//			HWND		hwnd
//			int			active				当前帧是否鼠标点击左键
//			int			Flag_MenuSelected
//返回值：	无
//		
void MUS_RenewMouse(HWND hwnd, int active, int Flag_MenuSelected)
{
	...
	int rect_err;					//鼠标是否在当前窗口中	
	static int activ_bak = 0;		//前回鼠标是否点击左键
	...
	MouseStruct.no =  - 1;
	if (MouseStruct.active)//鼠标有点击动作
	{
		if (!rect_err)//鼠标不在当前窗口中
		{
...
{
	BOOL ret = FALSE;
	if (0 <= MouseStruct.mx && MouseStruct.mx < MouseStruct.disp_w && 0 <= MouseStruct.my && MouseStruct.my <
	MouseStruct.disp_h && MouseStruct.active)
	{
...
{
	BOOL ret = FALSE;
	if (0 <= MouseStruct.mx && MouseStruct.mx < MouseStruct.disp_w && 0 <= MouseStruct.my 
		  && MouseStruct.my < MouseStruct.disp_h && MouseStruct.active)
	{
...
//-------------------------------------------------------------------------
//函数名：	MUS_SetMouseLayer
//功能：	设定鼠标控制的图层
//
//输入参数：
//			类型		参数名			意义
//			int			lno				要设定的鼠标控制的图层索引号
//返回值：	无						
//	
...
//-------------------------------------------------------------------------
//函数名：	MUS_ResetMouseRect_Layer
//功能：	设定当前层内所有对象不可点击（可点击对象结构初始化）
//
//输入参数：			
//			类型		参数名			意义
//			int			lno				当前层号
